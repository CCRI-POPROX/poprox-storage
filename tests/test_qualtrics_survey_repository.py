...
from poprox_storage.concepts.qualtrics_survey import Qualtrics_survey_response

import pytest
import os
from sqlalchemy import create_engine, insert, select, MetaData, Table, null, text
...
def test_get_active_survey(pg_url: str):
    ...
    with engine.connect() as conn:
        ...
        conn.execute(
            text(
                f"insert into qualtrics_surveys(qualtrics_id, base_url) values ('first', 'first');"
            )
        )
        conn.execute(
            text(
                f"insert into qualtrics_surveys(qualtrics_id, base_url, active) values ('second', 'second', false);"
            )
        )
        conn.commit()
...
def test_create_survey_instance(pg_url: str):
    ...
    with engine.connect() as conn:
        ...
        conn.execute(
            text(
                f"insert into accounts(account_id, email, status, source) values ('{uuid}', 'd@g.com', '', '');"
            )
        )
        ...
        results = conn.execute(
            text("select * from qualtrics_survey_instances;")
        ).fetchall()
        assert 1 == len(results)
...
def test_create_survey_response(pg_url: str):
    ...
    with engine.connect() as conn:
        ...
        conn.execute(
            text(
                f"insert into accounts(account_id, email, status, source) values ('{account_uuid}', 'd@g.com', '', '');"
            )
        )
        ...
        results = conn.execute(
            text("select * from qualtrics_survey_responses;")
        ).fetchall()
        assert 1 == len(results)
